...
Feature:
  #Bir kullanıcı olarak "Ev Değişimi" ilan oluşturma sayfasında, 10. stepte en az 5 en fazla 10 tane resim ekleyebilmeliyim.
  ...
  Scenario: It should be able to add at least 5 images in step 10 on the "Ev Değişimi"  ad creation page.
    Then user shouldn't save the pictures without adding minimum 5 pictures

  Scenario: It should be able to add at most 10 images in step 10 on the "Ev Değişimi" ad creation page.
    When user adds 8 pictures on the home change page
    Then user asserts that it can't be upload more images

  Scenario: After adding 5 images "Resimleri Kaydet" button must be clickable.
