Feature: As a user, I should be able to enter the "Remarks" information in step 6 of "Rent Room & Exchange" advertisement preparation.
  ...
  Background:
    ...
    Then user navigates to page 6 in Room Exchange
@wip
  Scenario: user should see the AÇIKLAMALAR header on the HomeExchange Page in 6th step
    Then user should see the "Açıklamalar" header on the 6th Page

  Scenario: user should see the explanation on the HomeExchange Page
    Then verify "Evinizle ve/veya Odanızla ilgili açıklamaları bu kısma yazabilirsiniz. Evinizle ve/veya odanızla ilgili belirtmek istediğiniz özel bir durum, özellik, ayrıcalık, avantaj veya dezavantaj varsa bu kısımda açıklayabilirsiniz." text is displayed on the page
  ...
  Scenario Outline: user should send keys to explanation input box
    When user enters "<textRoom>" explanation to the inputbox on the 6th page
    Then verify input box accept "<textRoom>" explanation correctly on the 6th page
    Examples:
      |textRoom|
      |Room My Sweet Room|
      |123456789/*-+|
